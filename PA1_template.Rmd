...
2. 5-minute interval of `r format(maxinterval, "%H:%M")`, on average across all the days in the dataset, contains the maximum number of steps of `r maxstep`. 
...
```
    ...
    There is totally `r NAcount` 'NA' values in the dataset.
...
```
    ...
    *The mean of the total number of steps taken per day is `r meanstepnew`    
    *The median of the total number of steps taken per day is `r medianstepnew`    
...
```
    ...
    Remember without filling the missing data, the mean is `r meanstep` and the meadian is `r medianstep`, those two values are not the same. However, after we refill the data, the mean and median values are closer. From the histogram, we can see the distribution gets centered.
...
```
    ...
    We can see that the patterns between weekdays and weekends are similar, even though not exactly the same.
